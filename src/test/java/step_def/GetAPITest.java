...
public class GetAPITest extends TestBase {
	...
	public void I_validate_the_response_statuscode_200() {
	    ...
	    public void send_the_request_to_the_server_with_page_number_as(int pageNumber) throws Throwable {
	    	resp = RestAssured.given()
		            .header("Accept", "application/json")
		            .header("Content-Type", "application/json")
	    ...
	    public void i_verify_the_respnse_status_code(int respstatuscode) throws Throwable {
	    	int respCode=resp.getStatusCode();
			Assertions.assertEquals(respCode, respstatuscode);
	    }

	    @Then("^I verify the first user record having email as \"([^\"]*)\"$")
	    public void i_verify_the_first_user_record_having_email_as_something(String emailid) throws Throwable {
	       String actemailid=resp.path("data[0].email");
	       Assertions.assertEquals(actemailid, emailid);
	    }
